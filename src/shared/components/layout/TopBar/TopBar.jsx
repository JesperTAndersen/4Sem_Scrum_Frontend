...
import { getGreetingWelcomeMessage, getTodaysFormattedDate } from '../../../utils/dateHelpers';
...
const AdminTopBar = ({ snapshot }) => {
  ...
  const pageTitle = routeLabels.find((r) => pathname.startsWith(r.route))?.label ?? 'MiseOS';

  return (
    <div className={styles.topBarContainer}>
      <div className={styles.welcomeContainer}>
        <span className={styles.greeting}>{`${greeting}, ${user.firstName}!`}</span>
      ...
      <div className={styles.rightSection}>
        <div className={styles.bellWrapper}>
          <button className={styles.bellBtn} onClick={() => setShowBell((p) => !p)}>
          ...
          {showBell && (
            <>
              <div className={styles.bellOverlay} onClick={() => setShowBell(false)} />
                  ...
                  <>
                    {snapshot?.pendingIngredientRequests > 0 && (
                      <button
                        ...
                        onClick={() => { navigate('/admin/ingredient-requests'); setShowBell(false); }}
                      >
                        <span className={styles.bellCount}>{snapshot.pendingIngredientRequests}</span>
                    ...
                    {snapshot?.pendingDishSuggestions > 0 && (
                      <button
                        className={styles.bellItem}
                        onClick={() => { navigate('/admin/dish-suggestions'); setShowBell(false); }}
                      >
                        <span className={styles.bellCount}>{snapshot.pendingDishSuggestions}</span>
        ...
        <div onClick={() => navigate('/admin/profile')} className={styles.profileContainer}>
